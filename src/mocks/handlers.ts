...
import { DMSMockdata } from '@src/components/dms/IntialState'

function findItemByName(files, name) {
...
export const handlers = [
	...
	graphql.query('GetFiles', ({ query, variables }) => {
		const { fileName } = variables
		const files = findItemByName(DMSMockdata.files, fileName)
		return HttpResponse.json({
			data: {
				id: 0,
				name: 'root',
